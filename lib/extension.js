...
const DEFAULT_MERMAID_LIBRARY_URL = 'https://cdn.jsdelivr.net/npm/mermaid/dist/mermaid.min.js'
...
const appendMermaidScript = (config) => ({uiCatalog}) => {
    ...
    const hbsContent = `<script src="${mermaidLibraryUrl}"></script><script>mermaid.initialize(${mermaidInitializeOptions});</script>`
    // noinspection JSUnresolvedFunction
    uiCatalog.addFile({
...
const toBlock = (attrs, parent, source, context) => {
    ...
    block.lines = [`<div${idAttr} class="${classAttr}"><div class="mermaid content">${source}</div>${titleElement}</div>`]
