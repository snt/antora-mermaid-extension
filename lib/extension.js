...
module.exports.register = (context, {config}) => {
    const logger = context.getLogger('antora-mermaid-extension')
    context.on('uiLoaded', appendMermaidScript(config,logger))
    context.on('beforeProcess', appendBlockProcessor)
}

const DEFAULT_MERMAID_LIBRARY_URL = 'https://cdn.jsdelivr.net/npm/mermaid@11/dist/mermaid.esm.min.mjs'
...
const appendMermaidScript = (config,logger) => ({uiCatalog}) => {
    ...
    let mio;
    if (config.mermaidInitializeOptionsRawTextOverride) {
        if (config.mermaidInitializeOptions) {
        ...
        mio = config.mermaidInitializeOptionsRawTextOverride
    } else {
        // noinspection JSUnresolvedVariable
        mio = JSON.stringify(config.mermaidInitializeOptions || DEFAULT_MERMAID_INITIALIZE_OPTIONS)
    }

    let mermaidRunCalling = '';
    if (config.mermaidRunOptions || config.mermaidRunOptionsRawTextOverride) {
        logger.info('mermaid.run() will be called')
        if (config.mermaidRunOptionsRawTextOverride) {
            if (config.mermaidRunOptions) {
                logger.warn('Both mermaidRunOptions and mermaidRunOptionsRawTextOverride are set. mermaidRunOptionsRawTextOverride is used.')
            }
            mermaidRunCalling = `mermaid.run(${config.mermaidRunOptionsRawTextOverride})`
        } else {
            mermaidRunCalling = `mermaid.run(${JSON.stringify(config.mermaidRunOptions)})`
        }
    }
    const elkLibraryUrl = config.elkLibraryUrl
    const elkPart = elkLibraryUrl
        ? `import elkLayouts from '${elkLibraryUrl}'; mermaid.registerLayoutLoaders(elkLayouts); `
        : ''
    const hbsContent = `<script type="module">import mermaid from '${mermaidLibraryUrl}'; ${elkPart}mermaid.initialize(${mio}); ${mermaidRunCalling}</script>`
    // noinspection JSUnresolvedFunction
    uiCatalog.addFile({
